...
Esse projeto foi desenvolvido durante o curso de Desenvolvimento Web da Trybe no móduto de Back-end. Neste foi solicitado:

1. **_Conteinerizar_** aplicações;
2. Criar uma conexão entre elas;
3. Orquestrar seu funcionamento.

Foi dada uma aplicação full-stack(docker/todo-app) neste repositório: um **aplicativo de tarefas**! Esta aplicação precisava ser conteinerizada para funcionar. Desenvolvi os arquivos de configuração para cada frente específica: `Front-end`, `Back-end` e, para um aplicativo de `teste` que valida se as aplicações estão se comunicando.
...
Segui os seguintes passos passados pela Trybe:
